...
const projects = [
  { name: "Portfolio workspace", detail: "12 components · Updated today", color: "#ff4d0a" },
  { name: "Mobile checkout", detail: "8 components · Updated 2 days ago", color: "#ff9b7b" },
  { name: "Dashboard kit", detail: "21 components · Updated Sep 12", color: "#222" },
];
      ...
      <div className="mypage-slider">
        <div className="mypage-slide-track" style={{ transform: `translateX(-${activeTab * 100}%)` }}>
          ...
          <section id="mypage-panel-2" className="mypage-slide" role="tabpanel" aria-hidden={activeTab !== 2}>
            <div className="mypage-panel-heading mypage-project-heading"><p>MY WORKSPACE</p><h2>내 프로젝트</h2></div>
            <div className="mypage-project-grid">
              {projects.map((project, index) => <article key={project.name}><div style={{ background: project.color }}><span>0{index + 1}</span><b>{project.name.slice(0, 1)}</b></div><h3>{project.name}</h3><p>{project.detail}</p><button type="button">프로젝트 열기 →</button></article>)}
            </div>
          </section>
        </div>
      </div>
