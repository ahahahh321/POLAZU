...
type HeaderProps = {
  ...
  activeNav?: "Explore" | "Editor";
};

export default function Header({ query = "", onSearch, activeNav = "Explore" }: HeaderProps) {
  return (
    <header className="common-header">
      <a href="/" className="common-logo" aria-label="tmp 홈">LOGO</a>
      <nav className="common-navigation" aria-label="주요 메뉴">
        <a href="/" aria-current={activeNav === "Explore" ? "page" : undefined}>Explore</a>
        <button type="button" disabled title="준비 중">Convert</button>
        <a href="/editor/" aria-current={activeNav === "Editor" ? "page" : undefined}>Editor</a>
        {["Community", "Pricing"].map((label) => (
